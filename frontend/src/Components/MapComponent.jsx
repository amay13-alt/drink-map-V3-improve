...
function MapComponent({ drinks, currentLang, onCopyAddr }) {
  ...
  drinks.forEach(drink => {
    // 💡 雙軌數據防禦：不管是假資料的 lat/lng，還是真資料庫的 latitude/longitude，通通都吃得到！
    const lat = drink.latitude !== undefined ? Number(drink.latitude) : Number(drink.lat);
    const lng = drink.longitude !== undefined ? Number(drink.longitude) : Number(drink.lng);
    const themeColor = drink.brand_color || drink.color || '#6366f1';

    if (lat && lng) {
      if (!storeGroups[drink.store]) {
        storeGroups[drink.store] = {
          store: drink.store,
          store_en: drink.store_en, 
          lat: lat,
          lng: lng,
          ...
          <Marker key={index} position={[shop.lat, shop.lng]} icon={createColoredIcon(shop.color)}>
            <Popup>
              <div style={{ minWidth: '185px', padding: '5px' }}>
                ...
                {/* 💡 修正：修復了 ${} 的拼寫錯誤，並更換為精準合規的國際標準 Google Maps 導航網址 */}
                <button 
                  onClick={() => window.open(`https://www.google.com/maps/?q=${shop.lat},${shop.lng}`)}
